...
{
protected:
  void RecordBenchmark(const std::string& name, std::chrono::microseconds duration, int iterations)
  {
...
TEST_F(CpuBenchmark, BreakSchedulerUpdatePerformance)
{
  ...
  auto duration = MeasureTimeWithWarmup([&]() { scheduler.Update(DurationMs(100)); }, kIterations);

  // 100000 updates should complete in < 500ms
  EXPECT_LT(duration.count(), 500000)
      << "Scheduler update took " << duration.count() << " us for " << kIterations << " iterations";
...
TEST_F(CpuBenchmark, MessageProviderPerformance)
{
  ...
  EXPECT_LT(duration.count(), 200000) << "Message retrieval took " << duration.count() << " us for "
