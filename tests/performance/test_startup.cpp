...
TEST_F(StartupTest, ConfigurationLoading)
{
  ...
  std::string json = R"({
        ...
        "idle": {
            ...
            "pause_on_idle": true,
            "reset_short_on_idle": true,
            "reset_short_threshold_seconds": 1200,
            "reset_long_on_idle": true,
            "reset_long_threshold_seconds": 1200,
            "show_timer": false
        },
        "notification": {
            "enabled": true,
...
TEST_F(StartupTest, DefaultConfigCreation)
{
  ...
  EXPECT_LT(duration.count(), 1000) << "Default config creation took " << duration.count() << " us";
...
TEST_F(StartupTest, StateMachineFullSetup)
{
  ...
  // Register callback
  sm.SetOnStateChange(
      [](State from, State to, const Event& event)
      {
        (void)from;
        (void)to;
        (void)event;
