...
{
public:
  explicit BlinkBreakToastHandler(std::shared_ptr<std::function<void(NotificationAction)>> callback)
      : callback_(std::move(callback))
  {
  }

  void toastActivated() const override { Dispatch(NotificationAction::Clicked); }
  ...
  void toastActivated(std::wstring) const override { Dispatch(NotificationAction::Clicked); }
  ...
  void toastFailed() const override { spdlog::error("WinToast: toast failed"); }
        ...
        while (true)
        {
          std::function<void()> task;
          {
            ...
            queue_cv_.wait(queue_lock,
                           [this]() { return !tasks_.empty() || !worker_running_.load(); });
...
bool NotificationManagerWin::Initialize()
{
  ...
  EnqueueTask(
      [this, promise]()
      {
        ...
        const auto aumi = WinToastLib::WinToast::configureAUMI(L"BlinkBreak", L"BlinkBreak",
                                                               L"BreakReminder", L"1.0");
