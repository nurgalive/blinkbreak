...
void TestHarness::SetupCallbacks() {
    ...
    scheduler_->SetOnWarning(
        [this](BreakType type, Duration time_until) { OnWarning(type, time_until); },
        current_config_.notification.warning_time);

    // Idle detector callbacks
    idle_detector_->SetOnIdle([this]() {
    ...
    dnd_detector_->SetOnDndChange([this](bool is_active) {
...
void TestHarness::UpdateConfig(const AppConfig& config) {
    ...
    scheduler_->SetOnWarning(
        [this](BreakType type, Duration time_until) { OnWarning(type, time_until); },
        config.notification.warning_time);
}
