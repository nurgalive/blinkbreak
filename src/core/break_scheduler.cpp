...
std::optional<Duration> BreakScheduler::GetTimeUntilNextBreak() const
{
  ...
  Duration short_remaining = short_config_.enabled ? short_timer_->GetRemaining() : Duration::max();
                                  ...
                                  const OverlayConfig& overlay)
{
  ...
  const bool snooze_duration_changed = (overlay.snooze_duration != overlay_config_.snooze_duration);
