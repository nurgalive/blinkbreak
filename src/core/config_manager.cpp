...
ConfigResult<AppConfig> ConfigManager::Load(const std::filesystem::path& path)
{
  ...
  {
    return std::unexpected(
        ConfigError{.message = "Failed to parse configuration: " + glz::format_error(error, buffer),
                    .field = path.string()});
  }
...
ConfigResult<void> ConfigManager::Save(const AppConfig& config, const std::filesystem::path& path)
{
  ...
  if (!parent.empty() && !std::filesystem::exists(parent))
  {
    ...
    if (ec)
    {
      return std::unexpected(ConfigError{.message = "Failed to create directory: " + ec.message(),
                                         .field = parent.string()});
    }
  }
...
std::vector<ConfigError> ConfigManager::Validate(const AppConfig& config) const
{
  ...
  {
    errors.push_back({.message = "Reset long break on idle threshold must be positive when enabled",
                                                            ...
                                                            const std::string& name) const
{
  ...
  {
    errors.push_back({.message = "Break interval must be positive", .field = name + ".interval"});
  }

  if (config.duration <= Duration::zero())
  {
    errors.push_back({.message = "Break duration must be positive", .field = name + ".duration"});
  }
  ...
  {
    errors.push_back({.message = "At least one message is required", .field = name + ".messages"});
  }
